...
function atualizar_dados($conn, $utilizador, $email, $genero, $distrito, $tempo){
$sql = "UPDATE `registo` SET `utilizador` = '$utilizador', `email` = '$email', `genero` = '$genero', `distritos` = '$distrito',`tempo` = '$tempo' WHERE `registo`.`id` = '$id';";
$result = mysqli_query($conn, $sql);
return $result;

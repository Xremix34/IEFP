...
function nome_do_genero($conn, $id_genero){
  $sql = "SELECT * FROM `registos` WHERE `id`='$id_genero'";
  $result = mysqli_query($conn, $sql);
...
function atualizar_dados($conn, $utilizador, $email, $genero, $distrito, $apikey, $tempo){
$sql = "UPDATE `registo` SET `utilizador` = '$utilizador', `email` = '$email', `genero` = '$genero', `distritos` = '$distrito',`apikey` = '$apikey',`tempo` = '$tempo' WHERE `registo`.`id` = '$id';";
$result = mysqli_query($conn, $sql);
return $result;
